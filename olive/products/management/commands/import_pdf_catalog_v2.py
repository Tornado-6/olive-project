...
class PDFPageProcessor:
    ...
    def __init__(self):
        self.image_quality = {"dpi": 300, "scale_factor": 2, "enhance_factor": 1.5}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("PDF catalog import completed successfully")
            )
    ...
    def create_product(self, product_info, images):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Failed to create product {product_info['name']}: {str(e)}"
                )
            )
    ...
    def categorize_product(self, product_info):
        ...
        try:
            ...
            default_subcategory = Category.objects.get(
                name="Custom Storage", parent=default_category
            )
        except Category.DoesNotExist:
            self.stdout.write(
                self.style.WARNING("Default category not found, creating it...")
            )
            default_category = Category.objects.create(name="Storage Solutions")
            default_subcategory = Category.objects.create(
                name="Custom Storage", parent=default_category
            )
        ...
        for main_cat, subcats in category_mapping.items():
            try:
                ...
                for subcat_name, keywords in subcats.items():
                    if any(
                        keyword in name or keyword in material for keyword in keywords
                    ):
                        try:
                            return Category.objects.get(
                                name=subcat_name, parent=main_category
                            )
                        except Category.DoesNotExist:
                            continue
            except Category.DoesNotExist:
    ...
    def generate_description(self, product_info):
        ...
        if len(description_parts) < 2:
            description_parts.append(
                "High-quality kitchen and home organization solution."
            )
        ...
        if os.path.exists(product_images_dir):
            for filename in os.listdir(product_images_dir):
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"Error deleting {file_path}: {e}")
                    )
    ...
    def ensure_category_structure(self):
        ...
        subcategories = {
            ...
            "Organization": ["Drawer Systems", "Trays", "Organizers"],
            "Hardware": [
                "Handles & Knobs",
                "Hinges",
                "Brackets & Supports",
            ...
            "Furniture": ["Cabinets", "Wardrobes", "Shelving Units"],
            "Kitchen Solutions": [
                "General Storage",
                "Pantry Solutions",
                "Corner Solutions",
        ...
        for main_cat in main_categories:
            category, created = Category.objects.get_or_create(
                name=main_cat, defaults={"parent": None}
            )
            if created:
            ...
            for sub_cat in subcategories.get(main_cat, []):
                sub_category, sub_created = Category.objects.get_or_create(
                    name=sub_cat, defaults={"parent": category}
                )
                if sub_created:
                    self.stdout.write(
                        f"Created subcategory: {sub_cat} under {main_cat}"
                    )
