...
def password_reset_request(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            try:
                send_mail(subject, strip_tags(email_html), 'noreply@olivestore.com',
                         [user.email], html_message=email_html)
                return redirect('users:password_reset_done')
            except BadHeaderError:
                messages.error(request, 'Invalid header found in email.')
        messages.success(request, 'Password reset instructions have been sent if the email exists.')
        return redirect('users:password_reset_done')
